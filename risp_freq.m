tic
% Frequency responses of the TRAS (with h_inf or mu controller)

Ts = 0.01;
% Generiamo il sistema open loop senza pesi di performance
sys_ol
% Otteniamo il closed loop system
cls = lft(sim_ic,K,2,4);
...
grid
temp1 = 'Singular Value Plot of the';
temp2 = ' Otput Sensitivity Transfer Function Matrix';
title([temp1,temp2])
xlabel('Frequency')
axis([10^(-8) pi/Ts -100 40]);
leg = legend('\sigma(T_{dy})','1/w_{p1}', '1/w_{p2}','Location', 'SouthWest');
leg.FontSize = 14;
hold off
%
figure(3)
