% Uncertain model of the Two Rotor Aerodynamical System (fig 18.6/7/8)

alpha_v = -0.1; %???NON DOVREBBE ESSERE NULLO?

% Azimuth
I_h = 1/37000; % kg.^m2 Moment of inertia of tail rotor
k_H_h_nom = 9.838910923224026*10^3;
k_H_h = ureal('k_H_h',k_H_h_nom,'Percentage',10);
k_F_h_nom = 2.129316621491640*10^(-5);
k_F_h = ureal('k_F_h',k_F_h_nom,'Percentage',10);
l_t = 0.216;
J_h_nom = 0.0237903793335*(cos(alpha_v))^2 + 0.0030096206665;
J_h = ureal('J_h',J_h_nom,'Percentage',10);
k_f_h_nom = 0.00588996198;
k_f_h = ureal('k_f_h',k_f_h_nom,'Percentage',5);
k_v_h_nom = -0.0178200378418;
k_v_h = ureal('k_v_h',k_v_h_nom,'Percentage',5);

% Pitch
I_v = 1/6100; % kg.^m2 Moment of inertia of main rotor
k_H_v_nom = 4.874573566076675*10^3;
k_H_v = ureal('k_H_v',k_H_v_nom,'Percentage',10);
k_F_v_nom = 3.077225697928733*10^(-4);
k_F_v = ureal('k_F_v',k_F_v_nom,'Percentage',10);
l_m = 0.202;
J_v = 0.0300580807997;
k_f_v_nom = 0.0127095;
k_f_v = ureal('k_f_v',k_f_v_nom,'Percentage',5);
k_h_v_nom = 0.00417495398545;
k_h_v = ureal('k_h_v',k_h_v_nom,'Percentage',5);
